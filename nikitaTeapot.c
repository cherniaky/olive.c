        ...
        if (olivec_tri_horz(&tri, WIDTH, y, &x1, &x2)) {
          for (int x = x1; x <= x2; ++x) {
            ...
            if (z > zbuffer[y * WIDTH + x]) {
              ...
              int u3 = det - u1 - u2; 
              Uv uv1 = olivec_uv(0 / v1.z, 0.5 / v1.z);
              Uv uv2 = olivec_uv(1 / v2.z, 0.7 / v2.z);
              Uv uv3 = olivec_uv(1 / v3.z, 0 / v3.z);

              Uv uv = olivec_uv(
                  uv1.u * u1 / det + uv2.u * u2 / det + uv3.u * u3 / det,
                  uv1.v * u1 / det + uv2.v * u2 / det + uv3.v * u3 / det);

              int nikita_x = uv.u / z * nikita.width;
              int nikita_y = uv.v / z * nikita.height;

              if (nikita_x < 0)
                nikita_x = 0;
              if ((size_t)nikita_x >= nikita.width)
                nikita_x = nikita.width - 1;
              if (nikita_y < 0)
                nikita_y = 0;
              if ((size_t)nikita_y >= nikita.height)
                nikita_y = nikita.height - 1;

              OLIVEC_PIXEL(oc, x, y) = OLIVEC_PIXEL(nikita, nikita_x, nikita_y);
