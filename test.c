...
bool record_test_case(const char *expected_file_path) {
  if (!stbi_write_png(expected_file_path, WIDTH, HEIGHT, 4, actual_pixels,
...
void test_fill_rect(void) {
  ...
  olivec_fill_rect(actual_pixels, WIDTH, HEIGHT, WIDTH / 2 - WIDTH / 8,
                   HEIGHT / 2 - HEIGHT / 8, WIDTH / 4, HEIGHT / 4, RED_COLOR);
  olivec_fill_rect(actual_pixels, WIDTH, HEIGHT, WIDTH - 1, HEIGHT - 1,
                   -WIDTH / 2, -HEIGHT / 2, GREEN_COLOR);
  olivec_fill_rect(actual_pixels, WIDTH, HEIGHT, -WIDTH / 4, -HEIGHT / 4,
                   WIDTH / 2, HEIGHT / 2, BLUE_COLOR);
}
...
void test_fill_circle(void) {
  ...
  olivec_fill_circle(actual_pixels, WIDTH, HEIGHT, WIDTH / 2, HEIGHT / 2,
                     WIDTH / 4, BLUE_COLOR);
  olivec_fill_circle(actual_pixels, WIDTH, HEIGHT, WIDTH * 3 / 4,
                     HEIGHT * 3 / 4, -WIDTH / 4, GREEN_COLOR);
}

void test_draw_line(void) {
  olivec_fill(actual_pixels, WIDTH, HEIGHT, BACKGROUND_COLOR);
  olivec_draw_line(actual_pixels, WIDTH, HEIGHT, 0, 0, WIDTH, HEIGHT,
                   RED_COLOR);
  olivec_draw_line(actual_pixels, WIDTH, HEIGHT, WIDTH, 0, 0, HEIGHT,
                   BLUE_COLOR);
  olivec_draw_line(actual_pixels, WIDTH, HEIGHT, WIDTH / 2, 0, WIDTH / 2,
                   HEIGHT, GREEN_COLOR);
}
...
void test_fill_triangle(void) {
...

  {
    ...
    olivec_fill_triangle(actual_pixels, WIDTH, HEIGHT, x1, y1, x2, y2, x3, y3,
                         RED_COLOR);
  }
