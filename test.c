...
Replay_Result replay_test_case(const char *program_path,
  ...
  for (size_t y = 0; y < min_size(actual_canvas.height, expected_canvas.height);
       ++y) {
    for (size_t x = 0; x < min_size(actual_canvas.width, expected_canvas.width);
         ++x) {
...
Olivec_Canvas test_hello_world_text_rendering(void) {
  ...
  olivec_fill(oc, BACKGROUND_COLOR);
  olivec_text(oc, text, oc.width / 2 - DEFAULT_FONT_WIDTH * size * text_len / 2,
              oc.height / 2 - DEFAULT_FONT_HEIGHT * size / 2, default_font,
              size, FOREGROUND_COLOR);
  return oc;
}
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < TEST_CASES_COUNT; ++i) {
    ...
    if (record) {
      if (!record_test_case(actual_canvas, test_cases[i].expected_file_path))
        return_defer(1);
    } else {
      if (replay_test_case(program_path, actual_canvas,
                           test_cases[i].expected_file_path,
