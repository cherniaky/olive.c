...
bool replay_test_case(const char *program_path, const char *file_path,
...

  {
    ...
    expected_pixels = (uint32_t *)stbi_load(file_path, &expected_width,
    ...
    if (failed) {
      fprintf(stderr,
              ...
              file_path);
      if (!stbi_write_png(failure_file_path, WIDTH, HEIGHT, 4, pixels,
                          sizeof(uint32_t) * WIDTH)) {
        fprintf(stderr, "ERROR: could not generate image diff %s: %s\n",
                failure_file_path, strerror(errno));
      } else {
        fprintf(stderr,
                "%s: HINT: See image diff %s for more info. The pixels with "
                "color %s are the ones that differ from the expected ones.\n",
...
#define DEFINE_TEST_CASE(name)                                                 \
  {                                                                            \
    .run = name, .file_path = TEST_DIR_PATH "/" #name ".png",                  \
    .failure_file_path = TEST_DIR_PATH "/" #name "_failure.png"                \
  }

void test_fill_rect(void) {
  olivec_fill(pixels, WIDTH, HEIGHT, BACKGROUND_COLOR);
  olivec_fill_rect(pixels, WIDTH, HEIGHT, WIDTH / 2 - WIDTH / 8,
                   HEIGHT / 2 - HEIGHT / 8, WIDTH / 4, HEIGHT / 4, RED_COLOR);
  olivec_fill_rect(pixels, WIDTH, HEIGHT, WIDTH - 1, HEIGHT - 1, -WIDTH / 2,
                   -HEIGHT / 2, GREEN_COLOR);
  olivec_fill_rect(pixels, WIDTH, HEIGHT, -WIDTH / 4, -HEIGHT / 4, WIDTH / 2,
                   HEIGHT / 2, BLUE_COLOR);
}
...
void test_fill_circle(void) {
  ...
  olivec_fill_circle(pixels, WIDTH, HEIGHT, WIDTH / 2, HEIGHT / 2, WIDTH / 4,
                     BLUE_COLOR);
  olivec_fill_circle(pixels, WIDTH, HEIGHT, WIDTH * 3 / 4, HEIGHT * 3 / 4,
                     -WIDTH / 4, GREEN_COLOR);
}
...
void test_fill_triangle(void) {
...

  {
    ...
    olivec_fill_triangle(pixels, WIDTH, HEIGHT, x1, y1, x2, y2, x3, y3,
                         RED_COLOR);
  }
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < TEST_CASES_COUNT; ++i) {
    ...
    if (record) {
      ...
      if (!replay_test_case(program_path, test_cases[i].file_path,
                            test_cases[i].failure_file_path))
        return 1;
    }
  }
  return 0;
